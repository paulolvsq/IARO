...
import sys
    ...
    # ---- Initialisations     ----
    # ---- ---- ---- ---- ---- ----
    # Ces fonctions sont toutes semblables et permettent de choisir la manière dont on souhaite créer les chemins
    # sans avoir à toucher dans le main
...
def initPath3(posPlayers,players,goalStates,wallStates):
    """
    Une tentative de calcul d'a* classique sans gestion de collision pour chacun des joueurs dans lequel on aurait fait par la suite
    avancer les joueurs par groupes qui ne se croisent pas : si un joueur croise à un moment le chemin d'un autre il commence après
    que le précédent ait fini, mais si deux joueurs ne se croisent pas ils peuvent progresser en simultané
    Cet essai n'a pas été terminé
    """
    listeIters = []
    playerGoal = [goalStates[i] for i in range(len(players))]
    for i in range(len(posPlayers)):
...
def initPath3D(posPlayers,players,goalStates,wallStates):
    """
    Initialise les chemins en considérant les chemins déjà générés par l'utilisation d'un a* gérant la dimension temporelle,
    sauf si ca implique de ne laisser aucun chemin disponible pour un agent, dans ce cas il ne tiendra pas compte de ces
    restrictions et les collisions potentiellement engendrées sont laissées à strategieCollision
    """
    listeIters = []
    playerGoal = [goalStates[i] for i in range(len(players))]
    for i in range(len(posPlayers)):
...
def initPath3DBis(posPlayers,players,goalStates,wallStates):
    """
    Initialise les chemins en considérant les chemins déjà générés par l'utilisation d'un a* gérant la dimension temporelle,
    (les différences avec initPath3D sont expliquées à la définition de la fonction astar3DBis)
    sauf si ca implique de ne laisser aucun chemin disponible pour un agent, dans ce cas il ne tiendra pas compte de ces
    restrictions et les collisions potentiellement engendrées sont laissées à strategieCollision
    """
    listeIters = []
...
def strategieSwap(numIter,numPlayer,listeIters,wallStates,players):
    ...
    L'idée était initialement de faire avancer les deux joueurs face à face sur la même itération pour qu'ils échangent
    de places sans jamais se retrouver sur la même case
    """
    numBloqueur = 0
    tmp = 0
    for i in range(len(listeIters)):
    ...
    # ---- ---- ---- ---- ---- ---- ----
    # Ces fonctions servent dans le main ou d'autres fonctions et permettent d'améliorer leur lisibilité ou de factoriser du code
...
def main():
    ...
    """
    Pour permettre de constater plus rapidement les cas dans lesquels notre code ne faisait pas ce qu'on souhaitait,
    nous avons créé une boucle infinie ici, qui permet de replacer des fioles une fois que toutes celles présentes ont
    été récupérées jusqu'à la fermeture de la fenetre ou une erreur
    On se sert des fonctions écrites au dessus pour augmenter la lisibilité ici et faciliter la modification + conservation
    des différents tests effectués.
    """
    accObjets = [] # liste des objets qui ont été ramassés (pour pouvoir les replacer sans devoir en recréer)
    while 1:
        ...
        i = 0
        while i < maxLen: # ici le choix du while est le plus simple, car la longueur d'un chemin peut évoluer à la suite d'une collision
            for j in range(nbPlayers): # on fait bouger chaque joueur séquentiellement
                ...
                if detection(players,j,next_pos): # S'il y a collision, on la gère avec la stratégie définie au dessus
                    listeIters[j] = strategieCollision(i,j,listeIters,wallStates,players) # Cette stratégie nous renvoie la nouvelle liste des mouvements pour le joueur actuel
                    listeLenParcours,maxLen  = calculLongueurs(listeIters) # On actualise les longueurs des différents chemins
                    row,col = listeIters[j][i]
                ...
                # Cette fonction permet de replacer les fioles et de limiter la zone de réapparition pour forcer les collisions à apparaitre plus souvent
                # Elle nous a évité de devoir dessiner plusieurs autres cartes en permettant de jouer sur d'autres plus petites avec plus de contraintes
                a = verification_objet((row,col),playerGoal,goalStates,j,posPlayers,score,accObjets,players,wallStates,12,19)
                #if a:      #Ce break n'est pas compatible avec l'exécution en mode multijoueur car il fait sauter une itération aux joueurs jouant après dans
                #    break  # la liste, créant des erreurs en les empechant parfois de récupérer leur fiole s'ils etaient dans leur derniere itération
            i+=1
        print(maxLen)
        #print ("scores:", score)
    pygame.quit()
